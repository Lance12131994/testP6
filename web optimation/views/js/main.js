...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    //使用getElementsByClassName代替原有方法querySelectorAll
    //同时将  document.getElementsByClassName("randomPizzaContainer")存储在变脸randomPizzaContainer
    var randomPizzaContainer = document.getElementsByClassName("randomPizzaContainer");
    var randomPizzaContainerLength = randomPizzaContainer.length;
    for (var i = 0; i < randomPizzaContainerLength; i++) {
      randomPizzaContainer[i].style.width = determineDx (size) + "px";
    }
  }
...
// https://www.igvita.com/slides/2012/devtools-tips-and-tricks/jank-demo.html
  ...
  for (var i = 0; i < itemsLength; i++) {
    scrooTopDivide = scrollTop / 1250;
    phase = 100 * (Math.sin(scrooTopDivide + (i % 5))) + items[i].basicLeft;
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  //根据窗口大小动态计算 Pizza 的数量
  var rows = Math.floor(screen.height/75);
  var pizzaQuantity = rows * cols;
  ...
  for (var i = 0; i < pizzaQuantity; i++) {
    ...
    //用getElementById代替querySelector
